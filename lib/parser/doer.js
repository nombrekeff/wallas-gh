...
WDoer.create = args_ =>
  new WDoer('create', args_,
    (args, context, vars) => {
      ...
      // if (args.body) {
      //   args.body = format(args.body, context)
      //   args.body = args.body.replace(varRegExp, replace_vars.bind(this, vars))
      // }
      // if (args.title) {
      //   args.title = format(args.title, context)
      //   args.title = args.title.replace(varRegExp, replace_vars.bind(this, vars))
      // }
      // if (args.labels) {
      //   args.labels.map(l => {
      //     l = format(l, context)
      ...
      if (args.what === 'comment') github.issues.comment(context, args, vars)
